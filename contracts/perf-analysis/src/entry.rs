...
fn bloom_test() -> Result<(), Error> {
    ...
    let bloom_filter = load_data(|buf, offset| syscalls::load_witness(buf, offset, 0, Source::Input)).map_err(|e| Error::from(e))?;

    debug!("First 10 bytes: {:?}", bloom_filter.get(..10));

    debug!("Restore bloom filter from bits.");
    let bf = BloomFilter::new_with_data(239527, 17, &bloom_filter);
